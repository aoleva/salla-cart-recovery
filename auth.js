...
function sessionMiddleware(req, res, next) {
  const session = verify(parseCookies(req).aoleva_session);
  if (!session?.merchantId) return res.status(401).json({ success: false, message: 'جلسة غير صالحة. افتح التطبيق من لوحة سلة مرة أخرى.' });
  req.merchantId = String(session.merchantId);
  next();
}

function setSessionCookie(res, merchantId) {
  const token = sign({ merchantId: String(merchantId), exp: Date.now() + 8 * 60 * 60 * 1000 });
...
module.exports = { sessionMiddleware, setSessionCookie };
